...
class SRRI:
    ...
    The volatility of the fund shall be computed, and then rescaled to a yearly basis, using the
    following standard method:
    volatility = sigma_{f} = \sqrt( (m/T-1) * \sum{t=1}{T}(r_f,t - r-mean_f)**2 )
    where the returns of the fund are measured over T non overlapping periods of the duration of
    1/m years. This means m=52 and T= 260 for weekly returns, and m=12 and T=60 for monthly
    returns; and where r-mean_f is the arithmetic mean of the returns of the fund over the T periods:
    """
    ...
    def risk_class(self):
        for i in SRRI.RISK_CLASSES:
            if self._obj >= SRRI.RISK_CLASSES[i]["low"] and self._obj < SRRI.RISK_CLASSES[i]["high"]:
                return i
