...
class DownsideRisk:
    ...
    def __init__(self, pandas_obj, decay_factor=0.05, window=252) -> None:
        ...
        self.window = window
        self.downside_risk = pandas_obj[pandas_obj < 0].ewm(alpha=decay_factor).std().apply(lambda x: x * window ** 0.5)
    ...
    def plot(self, ax=None, **kwargs):  # pragma: no cover
        ...
        ax.set_title(f"Exp. weighted downside risk ({self.window}) - Decay factor {self.decay_factor}", fontweight='bold')
        return ax
