...
class RollingVolatility:
    ...
    window: will be used in square root. If daily returns and we want annualized volatility
    then window = 252, if we have weekly returns and want annualized vol then window = 52.
    ...
    def __init__(self, pandas_obj, rolling_window, window=252):
        ...
        self.window = window
        self._obj = pandas_obj.fillna(method='pad').rolling(window=rolling_window).std().apply(lambda x: x * window ** 0.5)
